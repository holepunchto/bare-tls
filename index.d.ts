...
export interface TLSSocketOptions {
  /** Whether the socket acts as a TLS server or client; if `true`, `cert` and `key` must be provided. Defaults to `false`. */
  isServer?: boolean
  ...
  key?: ArrayBufferView
  /** The hostname to verify the server certificate against; DNS names are also sent as the SNI extension, while IP literals are matched against the certificate's IP SANs with SNI suppressed. Required for client sockets unless `rejectUnauthorized` is `false`. */
  host?: string
  /** Whether the client rejects connections when certificate verification fails. Defaults to `true`. */
  rejectUnauthorized?: boolean
  /** A `Buffer` of one or more PEM-encoded CA certificates; when provided, only these CAs are used for verification instead of the bundled Mozilla root certificates. Defaults to `null`. */
  ca?: ArrayBufferView
 ...
 * @param opts - Options applied to each incoming socket; the same as `TLSSocket`, plus any options supported by `bare-net`.
 ...
 * @param opts - Options passed to both the underlying TCP socket and `TLSSocket`; `port` is required and `host` defaults to `'localhost'`.
